...
export const api = {
  companies: {
    ...
    getFormScanJob: (jobId: string) =>
      axios.get<{ job_id: string; status: string; done: number; total: number; total_eligible: number; found: number; pre_skip_count?: number; error?: string | null }>(`/api/companies/scan-forms/${jobId}`).then(r => r.data),
